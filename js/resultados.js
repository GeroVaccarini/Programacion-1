...
.then(function (data) {
        ...
        if (artista.length == 0) {
            artista.innerHTML = '¡La cancion que quisiste buscar no existe!'
            busqueda.innerHTML = ''
           artista.innerHTML = '<h1>¡No se encontraron resultados!</h1>'
        }
        for (let i = 0; i < 4; i++) {
            listaArtistas.innerHTML += `<article class="artista"><a href="detail-artist.html?id=${artista[i].title}">
            <img class="imagen" src="${artista[i].cover_big}" alt="${ artista[i].name }"> </a> 
            <h3><a href="detail-artist.html?id=${artista[i].id}">${ artista[i].title}</a>
            </h3></article>`
        }
    })
